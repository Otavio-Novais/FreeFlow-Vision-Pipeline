...
from config import IMG_TEST_DIR, MODEL_WEIGHTS_PATH, OUTPUTS_DIR  # type: ignore
...
class VehicleDetector:
    ...
    def __init__(
        self, weights_path: str = MODEL_WEIGHTS_PATH, conf_threshold: float = 0.05
    ):
        """
    ...
    def crop_vehicle(self, image_path: str, bbox: list) -> np.ndarray:
        ...
        x1 = max(
            0, min(x1, w)
        )  # Utilizamos do min() para garantir que o modelo não ultrapasse do tamanho real da imagem (modelo diz que x1 está em 1050 mas o tamanho da imagem é no máximo 1000)
        y1 = max(
            0, min(y1, h)
        )  # Utilizamos do max() para evitar que o modelo diga que a placa começa em pixel inexistentes (modelo diz que y1 começa no -20)
        x2 = max(
            0, min(x2, w)
        )  # Esses casos podem acontecer devido a natureza de regressão matemática aplicada, objetos que estão na borda correm esse risco.
        y2 = max(0, min(y2, h))

        cropped = img[
            y1:y2, x1:x2
        ]  # Com base no box predito pelo nosso modelo acima, estamos recortando a imagem
        return cropped.copy()
